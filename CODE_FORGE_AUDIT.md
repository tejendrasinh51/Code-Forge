# Code Forge Audit (Codelets Collaborative Engine)

**Scope decision:** Codelets is a **frontend playground** (HTML/CSS/JS only, CodePen-style) with real-time collaboration. Multi-language ambitions are dropped — schema enums and editor language switches that imply otherwise should be pruned.

**Guest mode:** `/ducklets/[id]/guest` is **kept** as a "try the snapshot" sandbox. Private collaborative sessions must not be readable through it.

---

## TL;DR

The Codelets engine (internally powered by Ducklets schemas) is conceptually sound and the realtime layer works end-to-end: Y.js documents persist as base64 in Postgres, Hocuspocus rehydrates them on load, chat replays from the doc, presence is wired. The membership state machine (invite / request / accept) is implemented in the tRPC router.

The feature is **not launch-ready**. Three classes of problem dominate:

1. **Authentication is honor-system.** The websocket trusts a `userId` query parameter; `ducklet.byId` is `publicProcedure`. Any private session is readable by anyone who knows its numeric ID.
...
#### FORGE-001 — Verify Hocuspocus connections with a real session token
**P0 · M**
...
Acceptance: a websocket connection with no token, a forged token, or a token for a different session is rejected with "Access denied."

#### FORGE-002 — Stop swallowing `onAuthenticate` errors
**P0 · XS**
...
#### FORGE-003 — Gate `ducklet.byId` with an authorization check
**P0 · S**
`packages/api/src/router/ducklet.ts:74` is `publicProcedure`. The handler does have visibility logic but `yjsData` is returned in the response (consumed by the guest page), so any caller who knows a numeric ID can fetch a private session's full source.

- Keep `publicProcedure` but: if the session is private, require an authenticated session AND an `ownerId === userId` or active membership row before returning `yjsData`. If unauthorized, either 403 or return the metadata without `yjsData`.
...
#### FORGE-004 — Implement (or remove) the anonymous-user branch in `onAuthenticate`
**P0 · XS**
`apps/hocuspocus-server/src/index.ts:38-41` is a 4-line comment with no code. Once FORGE-001 lands, the entire branch is moot — delete it. Until then, replace the comment with an explicit `throw new Error("Anonymous connections not allowed")`.

Acceptance: the file has no skeleton TODO comment; behavior is explicit.

#### FORGE-005 — Validate awareness (presence) updates against the server-verified user
**P0 · S**
...
#### FORGE-006 — Sanitize `headScripts` in preview render
**P0 · S**
...
Acceptance: a session with `headScripts = "<script>fetch('//evil')</script>"` does not execute the script during screenshot generation.

---

### P1 — Bugs the next user will hit

#### FORGE-007 — Pass `readOnly` to CSS and JS editors
**P1 · XS**
...
#### FORGE-008 — Wire `onVisibilityChange` from share modal
**P1 · XS**
...
#### FORGE-009 — Return `ownerId` from `ducklet.list` and gate the Delete dropdown
**P1 · XS**
...
#### FORGE-010 — Fix list pagination
**P1 · S**
`apps/web/src/app/(main)/ducklets/page.tsx:88` hardcodes `limit: 50` with no "load more." The 51st session is invisible.
...
#### FORGE-011 — Handle the "private + non-member" navigation case
**P1 · S**
`apps/web/src/app/(main)/ducklets/[id]/page.tsx:122-148` — a non-member visiting a private session URL gets stuck on "Connecting to room..." with no error. The redirect to `/guest` only happens when `isPublic` is true.

- If the user is not a member and the session is private, render an explicit "You don't have access to this session" screen with a Request Access button (only if logged in) or a Sign In CTA.

Acceptance: a non-member visitor sees a clear page, never an infinite spinner.

#### FORGE-012 — Add the `ducklet_member` primary key + UNIQUE(duckletId, userId)
**P1 · S**
...
#### FORGE-013 — Remove the `context.getConnections()` call in `onStoreDocument`
**P1 · S**
...
#### FORGE-015 — Add a Docker service for hocuspocus — _obsolete_
**P2 · S · won't fix**
...
#### FORGE-016 — Healthcheck endpoint
**P2 · XS**
...
#### FORGE-017 — Add the missing indexes
**P2 · S**
...
#### FORGE-018 — Stop accumulating R2 preview keys
**P2 · S**
...
Acceptance: R2 has one preview per session, not N.

#### FORGE-019 — Move preview generation off the store path
**P2 · M**
...
- Push a job to `packages/jobs` (BullMQ) with the session ID; debounce server-side (e.g., 60s per session) and run Puppeteer in a worker process.

Acceptance: rapid edits produce at most one screenshot per session per 60s window.

#### FORGE-020 — Bump `updatedAt` on Hocuspocus writes
**P2 · XS**
...
Acceptance: editing a session bumps its `updatedAt` and it moves to the top of "recent" lists.

#### FORGE-021 — CORS / origin restriction on the websocket
**P2 · S**
...
#### FORGE-022 — Drop multi-language scaffolding from the editor
**P3 · S · DONE**
...
#### FORGE-023 — Collapse the `ducklet_type` enum or drop the column
**P3 · S · DONE**
...
#### FORGE-024 — Remove the dead `"owner"` value from `memberRoleEnum`
**P3 · XS · DONE**
...
#### FORGE-025 — Rename "Pen Settings" to "Ducklet Settings"
**P3 · XS · DONE**

Renamed the dialog title and description in `settings-modal.tsx`.

#### FORGE-026 — Remove the `console.log("hi")` and other dev noise
**P3 · XS · DONE**
...
| ID | Title | Size | Notes |
|---|---|---|---|
| FORGE-028 | Rename in list dropdown + editor header | S | **DONE** — extended existing `update` mutation via `RenameDuckletDialog`, wired into list dropdown (owners) and editor header title click. |
| FORGE-029 | Fork / duplicate a session | S | New mutation that clones name + `yjsData` under the caller's ownership. |
| FORGE-030 | Search + sort on list page | M | **DONE** — `list` accepts `search` (ILIKE, escaped) + `sort: recent\\|updated\\|oldest`; UI has debounced search + sort Select. |
| FORGE-031 | Reconnect / offline indicator | XS | **DONE** — `ConnectionBadge` chip in header reads `isConnected` from `useSocketDucklet`, distinguishes "Live" / "Reconnecting…" / "Connecting…". |
| FORGE-032 | Mobile layout for the editor | M | **DONE** — `<md` switches editors to a 4-tab layout (HTML/CSS/JS/Preview); chat becomes a full-screen overlay. |
| FORGE-033 | Light-mode editor theme | S | Swap `oneDark` based on `useTheme()`. |
| FORGE-034 | Lazy-load CodeMirror + Y.js | S | `dynamic(() => import(...), { ssr: false })` around the editor. |
| FORGE-035 | Change member role after invite | S | New `updateMemberRole` mutation + UI in share modal. |
| FORGE-036 | Rate-limit `requestAccess` | S | Add a per-user-per-session cooldown (Redis counter). |
| FORGE-037 | Chat history read endpoint + pagination | M | New tRPC `ducklet.messages.list` reading `ducklet_message` ordered by `createdAt` with keyset pagination. |
| FORGE-038 | Snapshot / version history | L | Add `ducklet_snapshot` and periodic snapshotting in `onStoreDocument`. |
| FORGE-039 | Replace `window.confirm` delete with `AlertDialog` | XS | Match the rest of the app's UX. |
| FORGE-040 | Don't cascade-delete chat messages when a user is deleted | S | Change `onDelete: cascade` to `onDelete: set null` on `ducklet_message.userId`, render "deleted user" tombstone in chat. |
| FORGE-041 | Reduce N+1 in `byId` | S | Use a Drizzle relation query to fetch session + members + owner in one trip. |
| FORGE-042 | Reduce N+1 in `onStoreDocument` membership re-check | S | Cache membership per connection in `connection.context`. |
| FORGE-043 | Length caps on fields | S | Add zod `.max(...)` and DB `varchar(N)` where appropriate. |
| FORGE-044 | a11y improvements | S | Accessibility improvements to the editor. |
| FORGE-045 | Telemetry events | M | Add standard developer analytics layer. |
